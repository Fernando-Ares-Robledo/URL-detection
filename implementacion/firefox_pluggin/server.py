...
class ExtractorCaracteristicasURL:
    def __init__(self, db_config, geoip_db_path):
        ...
        self.cur.execute("""
        CREATE TABLE IF NOT EXISTS CaracteristicasURL32 (
    ...
    def verificar_online_y_redirigida(self, url):
        try:
            ...
            print(f"Error al comprobar la URL no esta online: {e}")
            return False, False
    ...
    def verificar_indice_google(self, url):
        ...
        try:
            ...
            print(f"Error al obtener resultados de búsqueda verificar google: {e}")
        return False

    def check_scripts_in_url(self, url):
        print("buscando scripts")
        try:
            ...
            print(f"Error fetching the URL comprobar scripts: {e}")
            return False
        finally:
            print("fin buscar scripts")
    def check_non_decodable_characters(self, url):
        print("no descodificables")
        try:
            ...
            print(f"Error fetching or decoding the URL non decodable characters: {e}")
            return True
        finally:
            print("fin no descodificables")
    ...
    def extraer_caracteristicas(self, url, maligna):
        ...
        if esta_online:
            ...
            caracteristicas.update({
                ...
                'edad_dominio': 0,
                'tiempo_restante': 0,
                'contiene_scripts': False,
                'contiene_caracteres_no_decodificables': False,
                'es_acortada': self.es_url_acortada(url),
            })
    ...
    def obtener_version_tls(self, url):
        print("obtener version tls")
        try:
            parsed_url = urlparse(url)
            print("parse url")
            contexto = ssl.create_default_context()
            print("tls contexto ok")
            with socket.create_connection((parsed_url.netloc, 443)) as sock:
                print("tls crear conexion ok")
                sock.settimeout(10)
                with contexto.wrap_socket(sock, server_hostname=parsed_url.netloc) as ssock:
                    print(f"Socket TLS envuelto correctamente, versión TLS: {ssock.version()}")
    ...
    def es_url_redirigida(self, url):
        try:
            ...
            #print(f"Error al comprobar la URL, redirige: {e}")
            return False
    ...
    def insertar_caracteristicas(self, caracteristicas):
        ...
        insert_query = sql.SQL("INSERT INTO CaracteristicasURL32 ({}) VALUES ({})").format(
            columns, values
        )
...
stacking_model = joblib.load('stacking_model3.pkl')
...
df = cargar_datos(db_config)
df = df.select_dtypes(exclude=['object', 'datetime64[ns]'])
df = df.drop(columns=['email_presente', 'tiene_hexadecimal'])
df = df.drop(columns=['id'])

df = df[df['esta_online']]
if 'esta_online' in df.columns:
    df = df.drop(columns=['esta_online'])
...
def predict():
    ...
    caracteristicas = extractor.extraer_caracteristicas(url, None)
    
    df_url = pd.DataFrame([caracteristicas])
    

    features_set1 = ['usa_https', 'esta_registrada', 'cantidad_letras'] + [col for col in df.columns if col not in ['usa_https', 'esta_registrada', 'cantidad_letras', 'maligna']]
    features_set2 = [col for col in df.columns if col not in ['usa_https', 'maligna']]
    features_set3 = [col for col in df.columns if col not in ['esta_registrada', 'maligna']]
    features_set4 = [col for col in df.columns if col not in ['cantidad_letras', 'maligna']]

    for feature_set in [features_set1, features_set2, features_set3, features_set4]:
        for feature in feature_set:
            if feature not in df_url.columns:
                df_url[feature] = 0  
    print("aaaaaaaaaaaaaaaaaaaaaaaaaa")            
    model_configs = [
        ('rf_model1', features_set1),
        ('lr_model1', features_set1),
        ('xgb_model1', features_set1),
        ('rf_model2', features_set2),
        ('lr_model2', features_set2),
        ('xgb_model2', features_set2),
        ('rf_model3', features_set3),
        ('lr_model3', features_set3),
        ('xgb_model3', features_set3),
        ('rf_model4', features_set4),
        ('lr_model4', features_set4),
        ('xgb_model4', features_set4)
    ]
    val_preds_combined = []

    for name, features in model_configs:
        model = joblib.load(f'{name}.pkl')
        print("bbbbbbbbbbbbbbbbbbbbbbbbbb")
        
        val_preds_combined.append(model.predict(df_url[features]))
        print("kkkkkkkkkkkkkkkkkkkkkkkkk")
    
    val_preds_combined = np.column_stack(val_preds_combined)
    
    final_prediction = stacking_model.predict(val_preds_combined)
    final_prediction_proba = stacking_model.predict_proba(val_preds_combined)
