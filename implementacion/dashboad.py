...
class ExtractorCaracteristicasURL:
    def __init__(self, db_config, geoip_db_path):
        ...
        self.cur.execute("""
        CREATE TABLE IF NOT EXISTS CaracteristicasURLs (
    ...
    def insertar_caracteristicas(self, caracteristicas):
        ...
        insert_query = sql.SQL("INSERT INTO CaracteristicasURLs ({}) VALUES ({})").format(
            columns, values
        )
...
st.title("Dashboard de Análisis de URLs")
url_input = st.text_input("Introduce la URL a analizar")
...
if url_input:
    ...
    with st.spinner("Extrayendo características..."):
        try:
            ...
            if caracteristicas['latitude'] and caracteristicas['longitude']:
